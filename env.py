...
class TwoPlayerPushEnv(gym.Env):
    """
    Two-player push environment compatible with gymnasium.Env

    Observation:
      Box(shape=(8,8,4), dtype=int8) - channels: [player1, player2, ball1, ball2] with 0/1 markers

    Action:
      MultiDiscrete([5,5]) : actions for player 1 and player 2 each in {0:stay,1:up,2:down,3:left,4:right}

    Reward:
      numpy array [r1, r2] where r1/r2 are rewards for each player (integer points when scoring)
    """
    metadata = {"render_modes": ["human"]}

    def __init__(self, grid_size: int = 8, max_steps: int = 100, goal_score: int = 20, seed: Optional[int] = None):
        super().__init__()
        self.size = grid_size
    ...
    def _place_random_empty(self):
        ...
        if not empties:
            ...
            return (self.np_random.randint(self.size), self.np_random.randint(self.size))
        return tuple(self.np_random.choice(len(empties)) and empties[0] if False else empties[self.np_random.randint(len(empties))])
    ...
    def _respawn_ball(self, ball_index: int):
        ...
        # remove the ball's current pos so it can potentially reuse it? They say when falls, appears random spot given there are no objects there.
        ...
        if not empties:
            ...
            newpos = (self.np_random.randint(self.size), self.np_random.randint(self.size))
        else:
    ...
    def step(self, action):
        """
        action: array-like of two ints [a1, a2]
        returns: observation, reward (array-like [r1,r2]), terminated (bool), truncated (bool), info (dict)
        """
        ...
        # clamp desired positions to in-bounds for 'moving into cells' check (we allow desired out-of-bounds as an attempt to move off board)
        ...
        # Helper flags
        p1_wants_p2cell = desired_p1 == self.p2_pos
        p2_wants_p1cell = desired_p2 == self.p1_pos
        ...
        # 1) Resolve both players competing for same empty cell -> random 50/50, other stays.
        ...
        # 3) Apply ball intended movements and resolve ball-ball collisions (perpendicular split or revert on swap).
        ...
        players_want_same_cell = (desired_p1 == desired_p2) and (desired_p1 != self.b1_pos) and (desired_p1 != self.b2_pos)
        p1_moves = True
        p2_moves = True
        ...
        # A push attempt happens when a player moves into the ball's cell (desired == ball_pos)
        b1_pushers = []
        b2_pushers = []
        ...
        if len(b1_pushers) == 1:
            ...
            if any(p[0] == 'p1' for p in b1_pushers) and any(p[0] == 'p2' for p in b1_pushers):
                ...
                # neither is owner (shouldn't happen for b1 because owner is p1), fallback random
        ...
        # If a ball has no pushers, but a player moves into its cell from a direction that isn't same-cell? That can't happen.
        ...
        if b1_will_be_pushed_by is not None:
            ...
            # If intended_b1 equals previous b2_pos and intended_b2 equals previous b1_pos -> potential swap; we'll handle later.
            b1_moves = True
        ...
        # If balls attempt to move into each other's previous positions (swap) and they were in same row or same column, then keep both in place.
        swap_attempt = (b1_moves and b2_moves and intended_b1 == prev_b2 and intended_b2 == prev_b1 and
                        (prev_b1[0] == prev_b2[0] or prev_b1[1] == prev_b2[1]))
        if swap_attempt:
        ...
        if b1_moves and b2_moves and intended_b1 == intended_b2:
            # Determine their push directions
            dir1 = b1_push_delta
            dir2 = b2_push_delta
        ...
        # ---------- If intended ball positions collide with players (not pushing), handle carefully ----------
        # If a ball intends to move into a cell where a player currently stands, that player would get pushed? The rules didn't define - players cannot be in same cell as objects.
        ...
        # If a player attempted to move into a ball cell and that ball successfully moved (b1_moves or b2_moves and intended pos != prev), then player can move into the ball's previous pos.
        ...
        if p2_moves:
            ...
            if wanted == prev_b1:
                ...
                if wanted != final_p1:  # prevent stepping into p1 new pos
                    ...
                    # if they would step into p1's new position (which moved), then resolve 50/50 random earlier — but if gets here, block.
                    final_p2 = self.p2_pos
        else:
            final_p2 = self.p2_pos
        ...
        info = {
            ...
            "prev_positions": {"p1": prev_p1, "p2": prev_p2, "b1": prev_b1, "b2": prev_b2},
            "intended": {"p1": desired_p1, "p2": desired_p2, "b1": intended_b1, "b2": intended_b2},
        }
